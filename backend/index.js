...
app.post("/update-restaurant", async (req, res) => {
  ...
  const restaurant = await conn.updateMany("restaurant", req.body, {
    filter: { userId: req.body.userId, placeId: req.body.placeId },
  });
  res.json(restaurant);
});
...
app.post("/get-restaurants", async (req, res) => {
  ...
  if (req.body.filter === "Saved") {
    const cursor = conn.getMany("restaurant", {
      filter: { userId: req.body.userId, starred: true },
    });
    const restaurants = [];
    ...
    // if (req.body.filter === "liked") {
    const cursor = conn.getMany("restaurant", {
      filter: { userId: req.body.userId, liked: true },
    });
    const restaurants = [];
    await cursor.forEach((item) => {
      restaurants.push(item);
...
app.post("/update-item/:restaurantId", async (req, res) => {
  ...
  const item = restaurant.itemsTried.find(
...
app.post("/google", async (req, res) => {
  ...
  const lat = req.body.lat.toString();
  const lng = req.body.lon.toString();
  const { userId } = req.body;
  console.log(lat, lng, userId);

  let google_url =
    "https://maps.googleapis.com/maps/api/place/textsearch/json?query=restaurant&location=";
  ...
  for (const restaurant of data.results) {
    // const conn = await Datastore.open();
    const restaurants = conn.getMany("restaurant", {
      filter: { userId, placeId: restaurant.place_id },
    });
